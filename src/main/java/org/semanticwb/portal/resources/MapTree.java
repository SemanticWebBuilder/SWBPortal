...
public class MapTree extends GenericResource {
	 ...
	 * 
	 * @see
	 * org.semanticwb.portal.api.GenericResource#processRequest(javax.servlet.http.
	 ...
	 */
	public void doData(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest)
			throws SWBResourceException, IOException {
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public void doChilds(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest)
			throws SWBResourceException, IOException {
		response.setHeader("mimetype", "text/json-comment-filtered");
		PrintWriter out = response.getWriter();
	...
	public String getData(WebPage tp) {
		...
		if (tp.getParent() == null)
			tptmp = tm.getHomePage();
		else
			tptmp = tp.getParent();

		ret.append(" {");
		ret.append(" id:'" + tptmp.getId() + "', carpeta:'true', name:'" + tptmp.getDisplayName() + "', tpurl:'"
				+ tptmp.getUrl() + "', type:'home', loaded:'true' "); // "+tptmp.getLevel()+"
			...
			if (ittp.hasNext()) {
				...
				while (ittp.hasNext()) {
					...
					if (null != tp1) {
						...
						ret.append("name:'" + tp1.getTitle() + "', tpurl:'" + tp1.getUrl() + "',  type:'"
								+ tp1.getParent().getId() + "' ");
						if (tp1.listChilds().hasNext()) {
							ret.append(", children: []");
						}
	 ...
	 * (non-Javadoc)
	 * 
	 * @see org.semanticwb.portal.api.GenericResource#doView(javax.servlet.http.
	 * HttpServletRequest, javax.servlet.http.HttpServletResponse,
	 * org.semanticwb.portal.api.SWBParamRequest)
	 */
	@Override
	public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramsRequest)
		...
		// custom function to handle drop for tree two. Items dropped onto nodes here
		// should be applied to the items attribute in this particular store.
		...
		// on tree two, we only want to drop on containers, not on items in the
		// containers
		out.println("	                function tree2CheckItemAcceptance(node,source) { ");
		out.println("	                        //return true; ");
		out.println("	                        var item = dijit.getEnclosingWidget(node).item; "); // var item =
																									// this.tree.getEnclosingWidget(node).item;
		...
		out.println(
				"      <li dojoType=\"dijit.MenuItem\" onClick=\"return getData(storetp.getValue(item, 'tpurl'), item)\">Reload</li> ");
		...
		out.println(
				"			<div dojoType=\"dijit.Tree\" id=\"mytreetp\" model=\"modeltp\" class=\"container\" childrenAttr=\"children\" dndController=\"dijit._tree.dndSource\" checkAcceptance=\"dndAccept\" checkItemAcceptance=\"tree2CheckItemAcceptance\"  >"); // dndController=\"dijit._tree.dndSource\"
																																																																		// //query=\"{type:
																																																																		// 'topic'}\"
																																																																		// labelAttr=\"name\"
																																																																		// label=\"WebSite\"
		...
		out.println("Data:");
		out.println("<textarea id=\"resultado\" name=\"resultado\" width=\"500px\" >");
