...
public class SWBARule extends GenericResource {
     ...
     * The combo att.
     */
    HashMap<String, Object> comboAtt = null;
     ...
     * @throws IOException
     */
    public void doUpdateRuleDefinition(HttpServletRequest request, HttpServletResponse response,
        ...
        if (callm != SWBResourceModes.Call_DIRECT || null == admUser) { // Can only be used with direct call and by
            // admin user
...
//                System.out.println("Rule XML:" + xml);
                r.setXml(xml);
...
//	}
    private void getRuleElements(Document doc, Element parent, JSONObject elemDef) {
        ...
        String type = elemDef.optString("condition");
        Element el = null;
//        System.out.println("Dom:" + SWBUtils.XML.domToXml(doc));
//        System.out.println("Parent:" + parent.getNodeName());
//        System.out.println("JSON:" + (elemDef != null ? elemDef.toString() : "null"));

        if (type.isEmpty()) { // Add condition tag
//            System.out.println("AGREGANDO CONDICION......");
            el = doc.createElement(elemDef.getString("id"));
            String op = elemDef.getString("operator");
            String val = elemDef.getString("value");
            ...
            JSONArray rules = elemDef.getJSONArray("rules");
            if (isNot) {
                notEle = doc.createElement("not");
                parent.appendChild(notEle);
            }

            if (("AND".equals(type) || "OR".equals(type)) && rules.length() > 1) {
                el = doc.createElement(type.toLowerCase());
                if (null != notEle) {
                    notEle.appendChild(el);
                } else {
                    parent.appendChild(el);
                }
                //el = parent;
     ...
     * @return JSON object containing rule definition or null if rule is
     * undefined.
     */
...
//        System.out.println("getElementsJSON:\n" + SWBUtils.XML.nodeToString(root));
        ...
        if (types.contains(tagName)) {
            if ("not".equals(tagName)) {
                ...
                if (root.getFirstChild() != null && root.getFirstChild().getNodeName().equals("and")) {
                    tagName = "and";
                    isNot = true;
                } else if (root.getFirstChild() != null && root.getFirstChild().getNodeName().equals("or")) {
                    tagName = "or";
                    isNot = true;
                }
                obj.put("not", isNot);
                obj.put("condition", tagName.toUpperCase());
                JSONArray rules = getElementRules((Element) root.getFirstChild());
                if (null != rules) {
                    obj.put("rules", rules);
                }
            } else {
...
//        System.out.println("===========================================\n" + SWBUtils.XML.nodeToString(root) + "\n========================================\n");
    ...
    /**
     * Process requests to get rule configuration options (conditions,
     * operators, values)
     *
     * @param request
     * @param response
     ...
     * @throws IOException
     */
    public void doGetRuleFilters(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
            throws SWBResourceException, IOException {
        response.setContentType("text/html; charset=UTF-8");
     ...
     * @throws IOException
     */
    public void doGetRuleJson(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
            throws SWBResourceException, IOException {
        response.setContentType("text/html; charset=UTF-8");
        response.setHeader("Cache-Control", "no-cache");
        response.setHeader("Pragma", "no-cache");
...
//            System.out.println("JSON: \n" + ret.toString());
     ...
     * @throws IOException
     */
    public void doEditRule(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
            throws SWBResourceException, IOException {
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
        ...
        PrintWriter out = response.getWriter();
        out.print(
                "<iframe style=\"border:0px; width:100%; height:100%; position:absolute; top:0px; left:0px;\" frameborder=\"0\" scrolling=\"no\" src=\""
                + paramRequest.getRenderUrl().setMode("editRule").setParameter("suri",
     ...
     * @throws IOException Signals that an I/O exception has occurred.
     */
    private void loadComboAttr(String tmid, String ruleid, SWBParamRequest paramRequest)
        ...
        vecOrderAtt = new ArrayList<>(1);
        // Agreando valores iniciales al HashMap como son isloged, isregistered,
        // language, device
        ...
        try {
            ...
            while (iteratt.hasNext()) {
                ...
                attnum++;
                log.debug("ListAttributes:" + usrAtt.getName() + ", " + attnum + ", objProp: "
                        + usrAtt.isObjectProperty());
                hmAttr = new HashMap<>();
                hmOper = new HashMap<>();
                hmValues = new HashMap<>();
            ...
            log.error(paramRequest.getLocaleString("msgErrorLoadingUserAttributeList") + ". SWBARules.loadComboAttr",
                    e);
        }
    }
