...
public class SWBACampResources extends GenericResource {
    ...
    public void doView(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest) throws SWBResourceException, IOException {
        ...
        HashMap<String, ResourceType> hmTypes = new HashMap<>();
        ...
        while(iteTypes.hasNext()){
            ...
            // Comienza cambio >>>>>>>>>>>>>>>>>>>
            HashMap<String, Resource> hmRec = new HashMap<>();
            Iterator<GenericObject> ite_ro = rrtype.listRelatedObjects();
            ...
            // Termina cambio <<<<<<<<<<<<<<<<<<<<<
            Iterator<String> iteRec = hmRec.keySet().iterator();
            boolean entro = false;
        ...
        if(act.equals("")){
            if(hmTypes.size()>0){
                ...
                rowColor="";
                cambiaColor = true;
                Iterator<ResourceType> iteRTypes = hmTypes.values().iterator();
                while(iteRTypes.hasNext()){
                    ResourceType rrtype = iteRTypes.next();
            ...
            if(act.equals("view")){
                ...
                // Comienza cambio >>>>>>>>>>>>>>>>>>>
                HashMap<String, Resource> hmRec = new HashMap<>();
                Iterator<GenericObject> ite_ro = restype.listRelatedObjects();
                while(ite_ro.hasNext())
                {
                    Object obj = ite_ro.next();
                    if(obj instanceof Resource)
