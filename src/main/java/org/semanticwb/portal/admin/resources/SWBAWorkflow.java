...
/**
 * Recurso para la administración de WebBuilder que permite la administración de
...
public class SWBAWorkflow extends GenericResource {
	...
	public void processAction(HttpServletRequest request, SWBActionResponse response)
		...
		if ("updateWorkflow".equals(act)) {
			...
			try {
				...
				GenericObject gobj = SWBPlatform.getSemanticMgr().getOntology()
						.getGenericObject(request.getParameter("suri"));
	...
	public String getXMLWorkflowData(PFlow flow, JSONObject payload) throws JSONException, SWBException {
		...
		for (int i = 0; i < arr.length(); i++) {
			...
			// Ignore start and end activities because SWBDocumentsToAuthorize expects XML
			// activities in this order
			if ((!name.isEmpty() && !description.isEmpty()) && !"AuthorActivity".equals(type)
					&& !"EndActivity".equals(type)) {
				...
				if (null != actRoles && actRoles.length() > 0) {
					for (int j = 0; j < actRoles.length(); j++) {
						...
						if (repo.isEmpty())
							repo = SWBContext.getAdminRepository().getId();
		...
		oldFlows.insertBefore(root, oldFlows.getFirstChild()); // This is because PFlowManager expects first child to be
																// the last version
	...
	public void getWorkflow(Element res, String tm, Document src) {
		if (src.getElementsByTagName("id").getLength() > 0) {
			...
			addElement("err", java.util.ResourceBundle.getBundle("org/semanticwb/portal/admin/resources/SWBAWorkflow")
					.getString("err1"), res);
		}
	}
	...
	public void getCatalogUsers(Element res, String tm) {
		...
		while (itUsers.hasNext()) {
			...
			try {
				addNode("user", "" + user.getId(), user.getName(), res);
			} catch (Exception ue) {
				ue.printStackTrace(System.out);
				LOG.error(ue);
			}
	 ...
	 * @param request
	 */
	public void update(Element res, Document src, User user, String tm, SWBParamRequest paramRequest,
		...
		try {
			...
			try {
				...
				LOG.error(e);
				e.printStackTrace(System.out);
				addElement("err", java.util.ResourceBundle
						.getBundle("org/semanticwb/portal/admin/resources/SWBAWorkflow").getString("msg1"), res);
			}
		} catch (Exception e) {
			LOG.error(e);
			e.printStackTrace(System.out);
			addElement("err", java.util.ResourceBundle.getBundle("org/semanticwb/portal/admin/resources/SWBAWorkflow")
					.getString("msg1") + " error: " + e.getMessage(), res);
		}
	 ...
	 * @return
	 */
	public Document getDocument(User user, Document src, String cmd, SWBParamRequest paramRequest,
			HttpServletRequest request) {
		Document dom = null;
		try {
			String tm = null;
	 ...
	 *             Signals that an I/O exception has occurred.
	 */
	public void doGateway(HttpServletRequest request, HttpServletResponse response, SWBParamRequest paramRequest)
			throws SWBResourceException, IOException {
		...
		if ("getWorkflow".equals(action)) {
			...
			SemanticObject obj = SWBPlatform.getSemanticMgr().getOntology()
					.getSemanticObject(request.getParameter("suri"));
		...
		for (int i = 0; i < nodes.getLength(); i++) {
			...
			if ("link".equals(nodeName)) {
				...
				if (null != to && !to.isEmpty())
					e.put("to", to);
				if (null != publish && !publish.isEmpty())
					e.put("publish", Boolean.valueOf(publish));
	...
	public JSONArray getResourceTypeCat(JSONArray filterRTypes, WebSite map, User user) throws JSONException {
		...
		String lang = "es";
		if (null != user && null != user.getLanguage())
		...
		while (elements.hasNext()) {
			...
			if (obj.getResourceMode() == ResourceType.MODE_CONTENT
					|| obj.getResourceMode() == ResourceType.MODE_SYSTEM) {
				JSONObject _obj = null;
				if (filterTypes.containsKey(obj.getId())) {
					_obj = filterTypes.get(obj.getId());
					_obj.put("selected", true);
				} else {
					_obj = createNodeObject(obj.getId(), obj.getDisplayTitle(lang),
							obj.getDescription() != null ? obj.getDescription() : "_");
	 ...
	 */
	private class OrdenaUsuarios implements Comparator<User> {
		...
		if (xml.getElementsByTagName("workflow").getLength() > 0) {
			...
			try {
				...
				if (docworkflows.getElementsByTagName("workflows").getLength() > 0) {
					...
					nodewf = (Element) workflows.insertBefore(nodewf,
							docworkflows.getElementsByTagName("workflow").item(0));
