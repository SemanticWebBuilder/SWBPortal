...
public class SWBFormMgr implements SWBForms {
	 ...
	 */
	public void init() {
		m_general = (PropertyGroup) SWBPlatform.getSemanticMgr().getOntology()
				.getGenericObject(SemanticVocabulary.SWBXF_URI + "pg_General");
	...
	private String getSelectClassElement() {
		...
		if (m_type.equals(TYPE_XHTML))
			XHTML = true;
		if (m_type.equals(TYPE_DOJO))
			DOJO = true;
		if (m_type.equals(TYPE_IPHONE))
			IPHONE = true;
	...
	public String getIdentifierElement() {
		...
		ret.append("        </td><td>\n");
		if (DOJO)
			ret.append("                <input type=\"text\" id=\"swb_create_id\" name=\"" + PRM_ID
					+ "\" dojoType=\"dijit.form.ValidationTextBox\" required=\"true\" promptMessage=\"Captura Identificador.\" isValid=\"return canCreateSemanticObject('"
					+ model + "','" + clsid
					+ "',this.textbox.value);\" invalidMessage=\"Identificador invalido.\" style=\"width:300px;\" trim=\"true\"/>\n");
		else
			ret.append("                <input type=\"text\" id=\"swb_create_id\" style=\"width:300px;\" name=\""
					+ PRM_ID + "\"/>\n");
		ret.append("	    </td></tr>\n");
		return ret.toString();
	}
	...
	public String getFormHiddens() {
		StringBuilder ret = new StringBuilder();
		if (m_obj != null)
			ret.append("    <input type=\"hidden\" name=\"" + PRM_URI + "\" value=\"" + m_obj.getURI() + "\"/>\n");
		...
		if (m_mode != null)
			ret.append("    <input type=\"hidden\" name=\"" + PRM_MODE + "\" value=\"" + m_mode + "\"/>\n");
		if (m_ref != null)
			ret.append("    <input type=\"hidden\" name=\"" + PRM_REF + "\" value=\"" + m_ref.getURI() + "\"/>\n");
	...
	public String renderForm(HttpServletRequest request) {
		...
		String onsubmit = "";
		if (m_onsubmit != null)
			onsubmit = " onsubmit=\"" + m_onsubmit + "\"";
		// si es dojo por default se manda por ajax
		if (m_onsubmit == null && submitByAjax)
		...
		if (DOJO)
			ret.append("<form id=\"" + frmname + "\" dojoType=\"dijit.form.Form\" class=\"swbform\" action=\""
					+ m_action + "\"" + onsubmit + " method=\"" + m_method.toLowerCase() + "\">\n");
		else
			ret.append("<form id=\"" + frmname + "\" class=\"swbform\" action=\"" + m_action + "\"" + onsubmit
					+ " method=\"" + m_method.toLowerCase() + "\">\n");
		...
		if (!m_mode.equals(MODE_CREATE)) {
			...
			while (itgp.hasNext()) {
				...
				// if(DOJO)
				// {
				// ret.append(" <fieldset dojoType=\"dijit.TitlePane\"
				// title=\""+group.getSemanticObject().getDisplayName(m_lang)+"\"
				// open=\""+first+"\">\n");
				// first=false;
				// }
				// else
			...
			// ret.append("<button dojoType='dijit.form.Button'
			...
			ret.append("	<fieldset>\n");
			// ret.append("
			// <legend>"+group.getSemanticObject().getDisplayName(m_lang)+"</legend>");
			ret.append("	    <table>\n");

			if (selectClass)
				ret.append(getSelectClassElement());
				...
				while (it.hasNext()) {
					...
					if (DOJO && !m_cls.isAutogenId() && prop.equals(m_cls.getDisplayNameProperty())) {
						ele.setAttribute("onkeyup",
								"if(dojo.byId('swb_create_id')!=null)dojo.byId('swb_create_id').value=replaceChars4Id(this.textbox.value);if(dojo.byId('swb_create_id')!=null)dijit.byId('swb_create_id').validate()");
					}
					renderProp(request, ret, prop, ele);
				}
			}
			...
			// type=\"submit\">Guardar</button>");
			// ret.append(" <button dojoType='dijit.form.Button'
			// onclick=\"dijit.byId('swbDialog').hide();\">Cancelar</button>");
			Iterator it = buttons.iterator();
			while (it.hasNext()) {
				Object aux = it.next();
				ret.append("    ");
	 ...
	 * 
	 * @deprecated Use {@link renderProp(HttpServletRequest, StringBuilder,
	 *             SemanticProperty, FormElement)} instead to enforce use of StringBuilder
	 * 
	 * @param request
	 *            the request
	 * @param ret
	 ...
	 *            the ele
	 */
	public void renderProp(HttpServletRequest request, StringBuilder ret, SemanticProperty prop, FormElement ele) {
		renderProp(request, ret, prop, prop.getName(), ele, m_mode);
	}
	 ...
	 * 
	 * @deprecated Use {@link renderProp(HttpServletRequest, StringBuilder,
	 * 			   SemanticProperty, FormElement, String)} instead to enforce use of StringBuilder
	 * @param request
	 *            the request
	 * @param ret
	 *            the ret
	 ...
	 * 
	 * @deprecated Use {@link renderProp(HttpServletRequest, StringBuilder,
	 * 			   SemanticProperty, String, FormElement, String)} instead to enforce use of StringBuilder
	 * @param request
	 *            the request
	 * @param ret
	 *            the ret
	 ...
	 *            the mode
	 */
	public void renderProp(HttpServletRequest request, StringBuffer ret, SemanticProperty prop, String propName,
			FormElement ele, String mode) {
	 ...
	 *            the mode
	 */
	public void renderProp(HttpServletRequest request, StringBuilder ret, SemanticProperty prop, String propName,
			FormElement ele, String mode) {
		renderProp(request, ret, prop, propName, ele, mode, null);
	}
	 ...
	 * 
	 * @deprecated Use {@link renderProp(HttpServletRequest, StringBuilder,
	 * 			   SemanticProperty, String, FormElement, String, String)} instead to enforce use of StringBuilder
	 * @param request
	 *            the request
	 * @param ret
	 *            the ret
	 ...
	 */
	public void renderProp(HttpServletRequest request, StringBuffer ret, SemanticProperty prop, String propName,
			FormElement ele, String mode, String label) {
		
		StringBuilder builder = new StringBuilder();
		renderProp(request, builder, prop, propName, ele, mode, label);
	 ...
	 *             the form validate exception
	 */
	public void validateElement(HttpServletRequest request, SemanticProperty prop, String propName)
			throws FormValidateException {
		SemanticObject obj = m_obj;
		if (obj == null) {
			obj = new SemanticObject(m_ref.getModel(), m_cls);
	 ...
	 * 
	 * @param filterHTMLTags
	 *            the filterHTMLTags to set
	 */
	...
	private String getCaptchaField(boolean isdojo) {
		StringBuilder ret = new StringBuilder();
		ret.append(
				"<tr><td align=\"right\"><label for=\"frmCaptchaValue\">Verificaci&oacute;n <em>*</em></label></td><td>\n");
		ret.append("<div id=\"captchaSnd\"></div><img src=\"" + SWBPlatform.getContextPath()
				+ "/frmprocess/requestCaptcha\" style=\"float:left;margin-left: 5px;margin-right: 30px;\" id=\"captchaimg\" />");
		ret.append("<a onclick=\"document.getElementById('captchaimg').src='" + SWBPlatform.getContextPath()
				+ "/frmprocess/requestCaptcha?'+ Math.random(); document.getElementById('frmCaptchaValue').value=''; return false;\">"
				+ getLocaleString("captchachgImg", m_lang) + "</a> ");
		ret.append("<a onclick=\"jBeep('" + SWBPlatform.getContextPath()
				+ "/frmprocess/requestSoundCaptcha?'+ Math.random())\">" + getLocaleString("scaptchasol", m_lang)
				+ "</a><br/>");
		...
		if (isdojo) {
			...
			ret.append(
					" isValid=\"return validateElement('frmCaptchaValue','/frmprocess/validCaptcha?none=1',this.textbox.value);\"");
